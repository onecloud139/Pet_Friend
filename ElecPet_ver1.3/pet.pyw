...
from PyQt5.QtWidgets import QApplication, QWidget, QSystemTrayIcon, QMenu, QActionGroup,QAction,QLabel
from PyQt5.QtCore import Qt, QTimer, QPointF,QThread,pyqtSignal,QPropertyAnimation,QSize
import requests
...
class DesktopPet(QWidget):
    def __init__(self):
        ...
        self.pet_image.setGeometry(0, 0, 200, 200)
        self.original_movie = QMovie("./pet.gif")
        self.close_movie = QMovie("./close_pet.gif")  
        self.original_movie.setScaledSize(QSize(200, 200))
        self.close_movie.setScaledSize(QSize(200, 200))
        self.original_movie.setCacheMode(QMovie.CacheAll)
        self.pet_image.setMovie(self.original_movie)
    ...
    def enable_chase_mode(self):
        ...
        self.timer.start(20)
        self.current_pos = QPointF(self.pos().x(), self.pos().y())
        self.original_movie.setPaused(False)
        self.close_movie.setPaused(True)
        self.pet_image.setMovie(self.original_movie)
    ...
    def enable_fixed_mode(self):
        ...
        self.original_movie.setPaused(True) 
        self.close_movie.setPaused(False)
        self.pet_image.setMovie(self.close_movie)
    ...
    def mousePressEvent(self, event):
        if not self.chase_mode and event.button() == Qt.LeftButton:
            self.drag_offset = event.pos()  # 记录鼠标相对窗口的位置
            self.dragging = True
    ...
    def mouseMoveEvent(self, event):
        if self.dragging and not self.chase_mode:
            ...
            self.move(new_pos)
            self.fixed_position = self.pos()  # 实时更新固定位置
        else:
    ...
    def update_position(self):
        ...
        if current_state != self.last_distance_state:
            if self.last_distance_state == "close":
                self.close_movie.setPaused(True)
            else:
                self.original_movie.setPaused(True)
            ...
            if current_state == "close":
                self.pet_image.setMovie(self.close_movie)
                self.close_movie.setPaused(False)  # 恢复播放
            else:
                self.pet_image.setMovie(self.original_movie)
                self.original_movie.setPaused(False)
